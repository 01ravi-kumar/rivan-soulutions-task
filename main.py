...
try:
  ...
  "body": "_m_=FindAPhysician&PhysicianSearch%24HDR0%24PhysicianName=&PhysicianSearch%24HDR0%24SpecialtyIDs=&PhysicianSearch%24HDR0%24Distance=5&PhysicianSearch%24HDR0%24ZipCodeSearch=&PhysicianSearch%24HDR0%24Keywords=&PhysicianSearch%24HDR0%24LanguageIDs=&PhysicianSearch%24HDR0%24Gender=&PhysicianSearch%24HDR0%24InsuranceIDs=&PhysicianSearch%24HDR0%24AffiliationIDs=&PhysicianSearch%24HDR0%24NewPatientsOnly=&PhysicianSearch%24HDR0%24InNetwork=&PhysicianSearch%24HDR0%24HasPhoto=&PhysicianSearch%24FTR01%24PagingID={}"
    ...
    with open('scrap_data.csv','w',newline="") as f:
        field_name = ["Full_Name", "Specialty", "Add_Specialty", "Full_Address", "Practice", "Address", "City", "State", "Zip", "Phone" , "URL"]
        ...
        #     doc_url = "https://www.stfrancismedicalcenter.com/find-a-provider/ryan-n-chan-pa/"
            ...
            try:
                Full_Name = doc_content.find("section", class_="system-style system-entry no-padding physician ui-repeater").get_text(strip=True)
                if flag:
                ...
                if flag:
                    print('Full_Name ===>',Full_Name)
            try:
                Specialty = doc_content.find('div',class_="sub-zone content-zone").find('li',class_="full flex-between-spaced-middle-wrap-block-550 mar-b-tiny ui-repeater").find('span').get_text(strip=True)
                if flag:
                ...
                if flag:
                    print('Specialty ===>',Specialty)
            try:
                Add_Specialty = doc_content.find('div',class_="sub-zone content-zone").find('li',class_="flex-between-spaced-middle-wrap-block-550 mar-b-tiny ui-repeater").find('span').get_text(strip=True)
                if flag:
                ...
                if flag:
                    print("Add_Specialty ===>")
            try:
                doc_address = doc_content.find('div',class_="sub-zone content-zone").find('div',class_="physician-locations content-style ui-repeater").find('ul',class_="full")
